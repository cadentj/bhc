    ...
    return (
        <div>
            ...
            <div className={"centered-flex page"} onClick={() => openPage("/narrative")}>
                ...
                <Box sx={{ width: "50%", display: "flex", flexDirection: "column", alignItems: "flex-end", pr: 4 , mt: -17}}>
                    <Box
                        sx={{ display: "flex", flexDirection: "column", alignItems: "flex-start" }}
                    >

                        {sectionNames.map((section, index) => (
                            <Box key={section} sx={{ display: "flex", alignItems: "center", mb: 2, cursor: "pointer" }} onClick={() => navigateToSection(section.toLowerCase())}>
                                {/* <Box sx={{ width: 15, height: 15, bgcolor: colors[index], borderRadius: "50%", mr: 1 }}></Box> */}
                                <ArrowForwardIcon sx={{ color: colors[index] }} />
